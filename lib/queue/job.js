...
exports.range = function(from, to, order, fn){
  if (order == 'desc') {
    redis.client().zrevrange('q:jobs', from, to, get(fn, order));
  } else {
...
exports.rangeByState = function(state, from, to, order, fn){
  if (order == 'desc') {
    redis.client().zrevrange('q:jobs:' + state, from, to, get(fn, order));
  } else {
...
exports.rangeByType = function(type, state, from, to, order, fn){
  if (order == 'desc') {
    redis.client().zrevrange('q:jobs:' + type + ':' + state, from, to, get(fn, order));
  } else {
  ...
  client.hgetall('q:job:' + job.id, function(err, hash){
    ...
    if (!hash) {
      ["inactive", "active", "failed", "complete", "delayed"].forEach(function(state) {
        client.zrem('q:jobs:' + state, job.id);
      });
