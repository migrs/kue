...
Worker.prototype.salvage = function(fn){
  var self = this;

  self.queue.on('salvage', function(job, time) {
    if (!self.processing && job._state == 'inactive' && job.type == self.type) {
      self.zpop(redis.keyPrefix + 'jobs:' + self.type + ':inactive', function(err, id) {
        if (err || !id || self.processing) return;
        console.log('salvage', job.id, job.type, job.data);
        self.process(job, fn);
      });
    }
  });
...
Worker.prototype.getJob = function(fn){
  ...
  client.blpop(redis.keyPrefix + self.type + ':jobs', 0, function(err) {
    if (err) return fn(err);		// SAE: Added to avoid crashing redis on zpop
    self.zpop(redis.keyPrefix + 'jobs:' + self.type + ':inactive', function(err, id){
      if (err) return fn(err);
      if (!id) return fn();
      if (self.processing) return fn();
      console.log('blpop', id, self.type);
      Job.get(id, fn);
    });
  });
};
